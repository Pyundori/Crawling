...
def make_html_body(args, vender_api):
    ...
    body.append("<hr/>")
    body.append("<h1>카카오 로그인 - 자체 토큰 발행(API) - POST</h1>")
    body.append(f"<p>api_url: http://{SERVER_IP}:5000/kakao/oauth2/callback</p>")
    body.append(params('kakaologin', args))
    body.append(return_value('kakaologin'))
    body.append("<hr/>")
    body.append("<h1>구글 로그인 - 자체 토큰 발행(API) - POST</h1>")
    body.append(f"<p>api_url: http://{SERVER_IP}:5000/google/oauth2/callback</p>")
    body.append(params('googlelogin', args))
    body.append(return_value('googlelogin'))
    body.append("<hr/>")
